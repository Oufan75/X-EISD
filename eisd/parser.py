...
def read_data(filenames, mode):
    ...
    if mode == 'exp':
        ...
        pre_data = pd.read_csv(filenames['pre'], header=None, delim_whitespace=True, index_col=0)   # shape: None, 67
        pre_data.index = range(pre_data.shape[0]) # just wanted to keep the indices 0-indexed

        noe_data = pd.read_csv(filenames['noe'], header=None, delim_whitespace=True, index_col=0)   # shape: None, 92
        ...
        jc_data.index = range(jc_data.shape[0]) # just wanted to keep the indices 0-indexed

        fret_data = pd.read_csv(filenames['fret'], header=None, delim_whitespace=True, index_col=0) 
        fret_data.index = range(fret_data.shape[0]) # just wanted to keep the indices 0-indexed
        ...
        return {
            ...
            'jc': Stack('jc', jc_data,
                        {'A': np.sqrt(0.14), 'B': np.sqrt(0.03), 'C':np.sqrt(0.08)}, #0.14, 0.03, 0.08
                        {'A': 6.51, 'B': -1.76, 'C': 1.6}),
            ...
            'cs': Stack('cs', pd.read_csv(filenames['cs'], header=None, delim_whitespace=True, index_col=0),  #shape: None, 262
                       {'C': 1.31, 'CA': 0.97, 'CB': 1.29, 'H': 0.38, 'HA': 0.29}, None), # reported from UCBShifts
            #'saxs': Stack('saxs', saxs_data, 0.00055, None)
        }

    elif mode == 'mcsce':
        pre_data = pd.read_csv(filenames['pre'], header=None, index_col=0)   
        noe_data = pd.read_csv(filenames['noe'], header=None, index_col=0)  
        jc_data = pd.read_csv(filenames['jc'], header=None, index_col=0)   
        fret_data = pd.read_csv(filenames['fret'], header=None, index_col=0)
        return {
            # property name: Stack(name, back calc data, sigma, mu)
            'pre': Stack('pre', pre_data.iloc[:4903], 0.0001, None),
            'noe': Stack('noe', noe_data.iloc[:4903], 0.0001, None),
            'jc': Stack('jc', jc_data.iloc[:4903],
                        {'A': np.sqrt(0.14), 'B': np.sqrt(0.03), 'C':np.sqrt(0.08)},
                        {'A': 6.51, 'B': -1.76, 'C': 1.6}),
            'fret': Stack('fret', fret_data.iloc[:7373], 0.0074, None),
            'cs': Stack('cs', pd.read_csv(filenames['cs'], header=None, index_col=0),  
                        {'C': 1.31, 'CA': 0.97, 'CB': 1.29, 'H': 0.38, 'HA': 0.29}, None),  # reported from UCBShifts
            #'saxs': Stack('saxs', saxs_data, 0.00055, None)
        }
    
    elif mode == 'ml':
        pre_data = pd.read_csv(filenames['pre'], header=None, index_col=0)   # shape: None, 67
        noe_data = pd.read_csv(filenames['noe'], header=None, index_col=0)   # shape: None, 92
        jc_data = pd.read_csv(filenames['jc'], header=None, index_col=0)   # shape: None, 47
        fret_data = pd.read_csv(filenames['fret'], header=None, index_col=0)
        cs_data = pd.read_csv(filenames['cs'], header=None, index_col=0) #shape: None, 262

        return {
            # property name: Stack(name, back calc data, sigma, mu)
            'pre': Stack('pre', pre_data.iloc[800:], 0.0001, None),
            'noe': Stack('noe', noe_data.iloc[800:], 0.0001, None),
            'jc': Stack('jc', jc_data.iloc[800:],
                        {'A': np.sqrt(0.14), 'B': np.sqrt(0.03), 'C':np.sqrt(0.08)},
                        {'A': 6.51, 'B': -1.76, 'C': 1.6}),
            'fret': Stack('fret', fret_data.iloc[800:], 0.0074, None),
            'cs': Stack('cs', cs_data.iloc[800:],  
                        {'C': 1.31, 'CA': 0.97, 'CB': 1.29, 'H': 0.38, 'HA': 0.29}, None),  # reported from UCBShifts
            #'saxs': Stack('saxs', saxs_data, 0.00055, None)
        }
        ...
        saxs_data = pd.read_csv(filenames['saxs'], header=None, delim_whitespace=True, index_col=0)   # shape: None, 37
        saxs_data.index = range(saxs_data.shape[0]) # just wanted to keep the indices 0-indexed
        ...
        return {
            ...
            'cs': Stack('cs', pd.read_csv(filenames['cs'], header=None, delim_whitespace=True, index_col=0).iloc[100:, :],  #shape: None, 262
                        {'C': 1.21, 'CA': 1.16, 'CB': 1.43, 'H': 0.52, 'HA': 0.29}, 
                        #{'C': 1.31, 'CA': 0.97, 'CB': 1.29, 'H': 0.38, 'HA': 0.22},  # SHIFTX2
                        None),
